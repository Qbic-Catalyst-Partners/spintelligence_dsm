...
export default function UserManagement() {
  ...
  const handleBulkUpload = async (event) => {
    ...
    const isAllowed = [".csv", ".xlsx", ".xls"].some((ext) => fileName.endsWith(ext));
    if (!isAllowed) {
      setUploadMessage("Please upload a CSV or Excel (.xlsx/.xls) file.");
      setUploadError(true);
      event.target.value = "";
      return;
    }
  ...
  const downloadBulkUploadTemplate = () => {
    try {
      ...
      link.href = "/UM_BU_Template.xlsx";
      link.setAttribute("download", "UM_BU_Template.xlsx");
