...
def reconstruct_table(results: List[OCRResult]) -> List[Dict[str, str]]:
    ...
    # 4. Prefer mean row; otherwise use computed mean fallback from numeric rows.
    rows_to_extract: List[List[OCRResult]]
    if mean_row is not None:
        rows_to_extract = [mean_row]
    else:
        rows_to_extract = selected_rows

    extracted_tables: List[Dict[str, str]] = []
    for selected_row in rows_to_extract:
