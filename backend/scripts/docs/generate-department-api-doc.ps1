...
$outPath = Join-Path $docsApiDir 'Department API Documentation.docx'
$tmp = Join-Path $repoRoot '__department_api_docx'
...
$endpointRows = @(
  ...
  @('GET', '/departments', 'Get paginated list of departments'),
  @('DELETE', '/departments/:id', 'Permanently delete a department after dependency checks'),
  @('PATCH', '/departments/:id/toggle', 'Toggle department active status')
)

$fieldRows = @(
  @('id', 'integer', 'Department primary key returned by the database'),
  @('name', 'string', 'Department name; required when creating a department'),
  @('is_active', 'boolean', 'Active status flag; defaults to true on create'),
  @('created_at', 'datetime', 'Timestamp returned for created/listed/toggled departments')
)

$createRows = @(
  @('name', 'string', 'Yes', 'Department name, compared case-insensitively for duplicates'),
  @('is_active', 'boolean', 'No', 'Initial active flag; defaults to true')
)

$queryRows = @(
  @('page', 'number', 'Optional page number for GET /departments; defaults to 1'),
  @('limit', 'number', 'Optional page size for GET /departments; defaults to 10')
)

$responseRows = @(
  @('POST /departments', '201', 'message, department'),
  @('GET /departments', '200', 'departments, total, page, limit'),
  @('DELETE /departments/:id', '200', 'message, department'),
  @('PATCH /departments/:id/toggle', '200', 'message, department')
)

$errorRows = @(
  @('400', 'Create validation error, duplicate department, or delete blocked by users/roles'),
  @('404', 'Department ID not found for delete or toggle'),
  @('500', 'Unhandled database or server error')
)

$dependencyRows = @(
  @('users.user_details', 'department_id', 'Delete is blocked when any employee uses the department'),
  @('rbac.role_departments', 'department_id', 'Delete is blocked when any role is assigned to the department'),
  @('rbac.departments', 'id, name, is_active, created_at', 'Primary table used by all Department API endpoints')
)

$body = ''
$body += Para 'Department API Documentation' 'Title' -Bold -Color '0B2545' -Size 36
$body += Para 'API reference for managing departments in routes/department.routes.js.' 'Subtitle' -Color '4B5563' -Size 22
$body += Para 'Overview' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para 'The Department API is mounted under /departments. It supports creating departments, reading a paginated department list, permanently deleting unused departments, and toggling the active status of a department.'
$body += Bullet 'Base route: /departments'
$body += Bullet 'Content type: application/json for create requests.'
$body += Bullet 'Server mount: server.js uses app.use("/departments", require("./routes/department.routes")).'
$body += Para 'Endpoints' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Table @('Method', 'Endpoint', 'Purpose') $endpointRows @(900, 3100, 5360)
$body += Para 'Department Object Fields' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Table @('Field', 'Type', 'Description') $fieldRows @(1900, 1600, 5860)
$body += Para 'Create Department' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para 'POST /departments creates a department after checking that the name does not already exist case-insensitively.'
$body += Table @('Field', 'Type', 'Required', 'Description') $createRows @(1700, 1500, 1200, 4960)
$body += Para 'Request Example' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para 'POST /departments' 'Code'
$body += Para '{ "name": "Finance", "is_active": true }' 'Code'
$body += Para 'Successful Create Response' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para '{ "message": "Department created successfully", "department": { "id": 1, "name": "Finance", "is_active": true, "created_at": "2026-06-10T00:00:00.000Z" } }' 'Code'
$body += Para 'List Departments' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para 'GET /departments returns departments ordered by id with simple offset/limit pagination.'
$body += Table @('Parameter', 'Type', 'Description') $queryRows @(2200, 1600, 5560)
$body += Para 'Request Example' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para 'GET /departments?page=1&limit=10' 'Code'
$body += Para 'Successful List Response' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para '{ "departments": [{ "id": 1, "name": "Finance", "is_active": true, "created_at": "2026-06-10T00:00:00.000Z" }], "total": 25, "page": 1, "limit": 10 }' 'Code'
$body += Para 'Delete Department' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para 'DELETE /departments/:id permanently removes a department only when it is not referenced by employees or role assignments.'
$body += Bullet 'Blocked when users.user_details has at least one row with the requested department_id.'
$body += Bullet 'Blocked when rbac.role_departments has at least one row with the requested department_id.'
$body += Para 'Request Example' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para 'DELETE /departments/3' 'Code'
$body += Para 'Successful Delete Response' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para '{ "message": "Department deleted successfully", "department": { "id": 3, "name": "Finance" } }' 'Code'
$body += Para 'Toggle Department Status' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para 'PATCH /departments/:id/toggle flips is_active using SET is_active = NOT is_active and returns the updated department row.'
$body += Para 'Request Example' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para 'PATCH /departments/3/toggle' 'Code'
$body += Para 'Successful Toggle Response' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para '{ "message": "Department status toggled successfully", "department": { "id": 3, "name": "Finance", "is_active": false, "created_at": "2026-06-10T00:00:00.000Z" } }' 'Code'
...
$body += Table @('Table', 'Relevant Columns', 'Usage') $dependencyRows @(2400, 2800, 4160)
$body += Para 'Implementation Notes' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Bullet 'The route uses the shared PostgreSQL client from connection.js.'
$body += Bullet 'Department name uniqueness is enforced at route level with LOWER(name)=LOWER($1) before insert.'
$body += Bullet 'The list endpoint performs a separate COUNT(*) query to return total records with paginated rows.'
$body += Bullet 'Delete is a hard delete, not a soft delete, so callers should use the toggle endpoint when only disabling a department.'
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dcmitype="http://purl.org/dc/dcmitype/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <dc:title>Department API Documentation</dc:title>
