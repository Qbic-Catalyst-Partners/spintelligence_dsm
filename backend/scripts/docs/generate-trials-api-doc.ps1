...
$outPath = Join-Path $docsApiDir 'Trials API Documentation.docx'
$tmp = Join-Path $repoRoot '__trials_api_docx'
...
$endpointRows = @(
  @('POST', '/trials', 'Create a new Trial entry'),
  @('GET', '/trials', 'List Trial entries with pagination'),
  @('GET', '/trials/master/counts', 'Count Name dropdown'),
  @('GET', '/trials/master/employees', 'Employee/user dropdown'),
  @('GET', '/trials/master/spinning-machines', 'Spinning machine dropdown'),
  @('GET', '/trials/master/autoconer-machines', 'Autoconer machine dropdown'),
  @('GET', '/trials/master/varieties', 'Variety dropdown')
)

$aliasRows = @(
  @('Count Names', '/master/count-dropdown, /master/count-names'),
  @('Employees', '/master/employee-dropdown, /master/employee-names, /master/user-names')
)

$payloadRows = @(
  @('date', 'date/string', 'Trial date'),
  @('spinning_machine', 'string', 'Spinning machine name'),
  @('autoconer_machine', 'string', 'Autoconer machine name'),
  @('count_name', 'string', 'Count Name'),
  @('purpose, trial_id_name, type, nature', 'string', 'Trial identity and category fields'),
  @('unit_no, raw_material, mixing', 'string', 'Trial context fields'),
  @('yarn_results, total_cuts, neps_cuts, shorts_cuts, long_cuts, thin_cuts', 'number/string', 'Yarn result and cut values'),
  @('cp, cm, ccp, ccm, jp, cvp', 'number/string', 'Quality/result metrics'),
  @('a1-a4, b1-b4, c1-c4, d1-d4, e, f, g, h1, h2, l1, l2', 'number/string', 'Reading grid fields'),
  @('user_id', 'number/string', 'User reference'),
  @('u_percent, cvm, cvm_cv_percent, cvm_10mtr, dr_1_5m', 'number/string', 'UQC/CVM fields'),
  @('thin/thick/neps totals, yarn_count, csp', 'number/string', 'Regular/HS imperfection and strength fields')
)

$queryRows = @(
  @('page', 'number', 'Trial list page number; defaults to 1'),
  @('limit', 'number', 'Trial list page size; defaults to 10'),
  @('prefix', 'string', 'Optional machine, variety, employee, or count search token'),
  @('count_prefix', 'string', 'Optional Count Name-specific search token')
)

$masterRows = @(
  @('Count Name', 'SQL Server dbo.Depot_CountMaster', 'Returns data, count_names, names, values, options'),
  @('Employee', 'createEmployeeMasterDropdown(sqlServer, "trials")', 'Returns employee dropdown data in shared helper format'),
  @('Spinning Machine', 'SQL Server dbo.MCMASTER joined with dbo.dept_mai', 'Filters department name containing spinning'),
  @('Autoconer Machine', 'SQL Server dbo.MCMASTER joined with dbo.dept_mai', 'Filters department name containing autoconer/autocone'),
  @('Variety', 'SQL Server dbo.VARIETY', 'Returns deduped variety names')
)

$responseRows = @(
  @('POST /trials', '201', 'Created trials.trials row'),
  @('GET /trials', '200', 'Array of trial rows ordered by date DESC'),
  @('GET master dropdowns', '200', 'source, data, names/options depending on endpoint')
)

$tableRows = @(
  @('trials.trials', 'Main Trial entry storage'),
  @('dbo.Depot_CountMaster', 'Count Name dropdown source'),
  @('dbo.MCMASTER', 'Spinning and Autoconer machine dropdown source'),
  @('dbo.dept_mai', 'Machine department filter source'),
  @('dbo.VARIETY', 'Variety dropdown source'),
  @('SQL Server employee master', 'Employee dropdown source through createEmployeeMasterDropdown')
)

$errorRows = @(
  @('500', 'Server/database error while creating or listing trials'),
  @('503', 'SQL Server is not configured for SQL Server-backed master endpoints')
)

$body = ''
$body += Para 'Trials API Documentation' 'Title' -Bold -Color '0B2545' -Size 36
$body += Para 'API reference for routes/trials.js, including Trial data entry, paginated listing, and SQL Server-backed master dropdowns.' 'Subtitle' -Color '4B5563' -Size 22
$body += Para 'Overview' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para 'The Trials API is mounted under /trials. It stores Trial data in PostgreSQL and exposes dropdown endpoints for count names, employees, spinning machines, autoconer machines, and varieties.'
$body += Bullet 'Base route: /trials'
$body += Bullet 'Content type: application/json for create requests.'
$body += Bullet 'Server mount: server.js uses app.use("/trials", require("./routes/trials")).'
$body += Bullet 'Master dropdown endpoints require SQL Server configuration.'
...
$body += Table @('Area', 'Aliases') $aliasRows @(2400, 6960)
$body += Para 'Create Payload Fields' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Table @('Field', 'Type', 'Description') $payloadRows @(3400, 1900, 4060)
$body += Para 'Common Query Parameters' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Table @('Parameter', 'Type', 'Description') $queryRows @(2200, 1700, 5460)
$body += Para 'Master Data Sources' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Table @('Dropdown', 'Source', 'Notes') $masterRows @(2100, 3600, 3660)
$body += Para 'Response Summary' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Table @('Endpoint', 'Status', 'Main Fields') $responseRows @(3000, 1200, 5160)
$body += Para 'Request Examples' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para 'POST /trials' 'Code'
$body += Para '{ "date": "2026-06-10", "spinning_machine": "RF 01", "autoconer_machine": "AC 01", "count_name": "40s Carded", "trial_id_name": "Trial-001", "purpose": "Quality validation" }' 'Code'
$body += Para 'GET /trials/master/spinning-machines?prefix=RF' 'Code'
...
$body += Bullet 'POST /trials inserts all supplied form fields directly into trials.trials and returns the created row.'
$body += Bullet 'GET /trials returns an array rather than a pagination wrapper; rows are ordered by date descending.'
$body += Bullet 'Machine endpoints return source, data, and names; machine rows include mc_no, mc_name, dept_code, and dept_name.'
$body += Bullet 'Count dropdowns return options with text, label, value, count_code, and count_name.'
$body += Bullet 'Variety results are deduplicated with dedupeVarieties before returning names.'
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dcmitype="http://purl.org/dc/dcmitype/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <dc:title>Trials API Documentation</dc:title>
