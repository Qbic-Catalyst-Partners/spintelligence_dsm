...
$outPath = Join-Path $docsApiDir 'Admin API Documentation.docx'
$tmp = Join-Path $repoRoot '__admin_api_docx'
...
$endpointRows = @(
  @('POST', '/admin/login', 'Authenticate the configured admin user and return an admin JWT')
)

$envRows = @(
  @('ADMIN_USERNAME', 'Required', 'Configured admin username'),
  @('ADMIN_PASSWORD', 'Required if ADMIN_PASSWORD_HASH is not set', 'Plain-text admin password fallback'),
  @('ADMIN_PASSWORD_HASH', 'Optional preferred', 'bcrypt hash used for password verification when present'),
  @('ADMIN_JWT_SECRET', 'Optional preferred', 'Secret used to sign admin JWTs'),
  @('JWT_SECRET', 'Fallback', 'Used when ADMIN_JWT_SECRET is not set')
)

$requestRows = @(
  @('username', 'string', 'Yes', 'Admin username; compared with ADMIN_USERNAME after trimming'),
  @('password', 'string', 'Yes', 'Admin password; checked against ADMIN_PASSWORD_HASH or ADMIN_PASSWORD')
)

$tokenRows = @(
  @('sub', 'Configured admin username'),
...
$responseRows = @(
  ...
  @('token', 'string', 'Signed admin JWT'),
  @('admin.username', 'string', 'Configured admin username'),
...
$errorRows = @(
  ...
  @('500', 'Admin credentials are not configured on the server, or server error')
)

$securityRows = @(
  @('Password hash priority', 'ADMIN_PASSWORD_HASH is used when present; ADMIN_PASSWORD is only used as fallback.'),
  @('JWT secret priority', 'ADMIN_JWT_SECRET is used first, then JWT_SECRET, then the hardcoded fallback admin_jwt_secret.'),
  @('Token lifetime', 'Generated admin tokens expire after 8 hours.'),
  @('Credential source', 'Admin credentials are read from environment variables, not from the database.'),
  @('Swagger tag', 'The route includes Swagger comments under the Admin tag.')
)

$body = ''
$body += Para 'Admin API Documentation' 'Title' -Bold -Color '0B2545' -Size 36
$body += Para 'API reference for dedicated admin authentication in routes/admin.js.' 'Subtitle' -Color '4B5563' -Size 22
$body += Para 'Overview' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para 'The Admin API provides a dedicated admin login endpoint. It validates credentials from environment variables and returns a signed admin JWT with an 8-hour expiry.'
$body += Bullet 'Base route: /admin'
$body += Bullet 'Server mount: server.js uses app.use("/admin", adminRouter).'
$body += Bullet 'Content type: application/json'
$body += Bullet 'Authentication for login: no existing token is required to call /admin/login.'
$body += Para 'Endpoints' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Table @('Method', 'Endpoint', 'Purpose') $endpointRows @(900, 2800, 5660)
$body += Para 'Environment Configuration' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Table @('Variable', 'Required', 'Description') $envRows @(2300, 2600, 4460)
$body += Para 'Admin Login' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para 'POST /admin/login authenticates the configured admin user. username and password must both be non-empty strings.'
$body += Table @('Field', 'Type', 'Required', 'Description') $requestRows @(1700, 1400, 1200, 5060)
$body += Para 'Request Example' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para '{ "username": "admin", "password": "Admin@123" }' 'Code'
$body += Para 'Successful Response Shape' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Table @('Field', 'Type', 'Description') $responseRows @(2300, 1400, 5660)
$body += Para 'Sample Success Response' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Para '{ "message": "Admin login successful", "token": "<admin-jwt>", "admin": { "username": "admin", "role": "admin" } }' 'Code'
$body += Para 'JWT Payload' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Table @('Claim', 'Value') $tokenRows @(2200, 7160)
$body += Para 'Security Behavior' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Table @('Item', 'Details') $securityRows @(2500, 6860)
$body += Para 'Error Responses' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Table @('Status', 'Meaning') $errorRows @(1200, 8160)
$body += Para 'Common Error Examples' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Bullet '400: { "message": "Username and password are required" }'
$body += Bullet '401: { "message": "Invalid admin credentials" }'
$body += Bullet '500: { "message": "Admin credentials are not configured on the server" }'
$body += Bullet '500: { "message": "Server error" }'
$body += Para 'Implementation Notes' 'Heading1' -Bold -Color '2E74B5' -Size 32
$body += Bullet 'getAdminJwtSecret() returns ADMIN_JWT_SECRET, then JWT_SECRET, then admin_jwt_secret.'
$body += Bullet 'isValidAdminPassword() verifies ADMIN_PASSWORD_HASH with bcrypt.compare when a hash exists.'
$body += Bullet 'If ADMIN_PASSWORD_HASH is absent, the supplied password is compared with ADMIN_PASSWORD.'
$body += Bullet 'The returned JWT includes role admin and auth_type admin.'
$body += Bullet 'The route imports adminOnly, but this file does not currently define any admin-only protected endpoints besides login.'
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dcmitype="http://purl.org/dc/dcmitype/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <dc:title>Admin API Documentation</dc:title>
