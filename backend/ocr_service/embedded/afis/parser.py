...
SKIP_ROW_PATTERNS = re.compile(
    r"^(std\.?\s*dev|cv\s*%|q99|min|max|usp|n\s*$|\[.*?\]|[\u2014\u2013\-]+$)",
    re.IGNORECASE,
)
...
MEAN_ROW_RE = re.compile(r"^(mean|average)\b", re.IGNORECASE)
...
def reconstruct_table(results: List[OCRResult]) -> List[Dict[str, str]]:
    ...
    mean_row: Optional[List[OCRResult]] = None
    for row in data_rows:
        if _is_mean_row(row):
            mean_row = row
            logger.info(f"[AFIS Parser] Found mean/average row: {[c.text for c in row]}")
            continue
    ...
    if not selected_rows and mean_row is None:
        logger.warning("[AFIS Parser] No usable data row found.")
        return []

    # 4. Build extracted rows and return ONE consolidated mean row for autofill.
    if mean_row is not None:
        mean_extracted = _extract_row_to_columns(mean_row, col_centers)
        logger.info("[AFIS Parser] Using explicit mean/average row from PDF.")
        return [mean_extracted]
    ...
    logger.info(f"[AFIS Parser] Computed consolidated mean row from {len(extracted_rows)} data rows.")
...
def _compute_column_means(extracted_rows: List[Dict[str, str]]) -> Dict[str, str]:
    ...
    for row in extracted_rows:
        for col, raw in row.items():
            txt = raw.strip()
            if not SIMPLE_NUMBER_RE.match(txt):
                continue
            value = float(txt)
...
def _is_mean_row(row: List[OCRResult]) -> bool:
    if not row:
        return False
    return bool(MEAN_ROW_RE.match(row[0].text.strip()))
